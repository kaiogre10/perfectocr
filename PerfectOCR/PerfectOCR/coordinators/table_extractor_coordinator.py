...
class TableExtractorCoordinator:
    def __init__(self, config: Dict, project_root: str, output_flags: Dict[str, bool]):
        ...
        # CORREGIDO: Pasa la config de outputs al handler
    ...
    def reconstruct_lines(self, ocr_results: Dict, base_name: str, output_dir: str) -> Dict[str, list]:
        ...
        # AGREGAR: Guardar la vista de texto para depuración si la bandera está activa
        if self.output_flags.get('debug_reconstructed_lines_text', False):
            self.json_output_handler.save_reconstructed_lines_text_view(
                reconstructed_data=reconstructed_lines_by_engine,
                output_dir=output_dir,
                file_name_with_extension=f"{base_name}_debug_reconstructed_text.json"
            )
    ...
    def extract_table_from_cleaned_lines(self, cleaned_lines: Dict[str, list], base_name: str, output_dir: str) -> Dict[str, any]:
        ...
        # --- Guardar versión debug de la structured_table ---
        if self.output_flags.get("debug_structured_table", False):
            self.json_output_handler.save(
                final_matrix,
                output_dir,
                f"{base_name}_debug_structured_table.json",
                output_type="debug_structured_table"
            )
        
        # Guardar output consolidado completo del geo_matrix
        geo_matrix_consolidated_path = None
        if self.output_flags.get("geo_matrix_consolidated", False):
            geo_matrix_consolidated_data = {
                "document_id": base_name,
                "processing_info": {
                    "header_detection_method": "ML" if use_ml_detector and header_words else "classic",
                    "tesseract_fallback_used": tesseract_fallback_used,
                    "table_boundaries": {
                        "header_y_min": y_min_band,
                        "header_y_max": y_max_band,
                        "table_end_y": y_min_table_end
                    },
                    "page_dimensions": page_dimensions
                },
                "header_elements": header_words,
                "header_semantic_types": [h.get('semantic_type', 'unknown') for h in header_words] if header_words else [],
                "table_matrix": final_matrix,
                "document_totals": document_totals,
                "monetary_totals_found": monetary_totals,
                "item_quantities_found": item_quantities,
                "table_body_lines_count": {
                    "paddle_lines": len(table_body_paddle_lines),
                    "tesseract_lines": len(table_body_tesseract_lines)
                },
                "matrix_dimensions": {
                    "rows": len(final_matrix),
                    "columns": len(final_matrix[0]) if final_matrix else 0
                }
            }
            
            geo_matrix_consolidated_path = self.json_output_handler.save(
                geo_matrix_consolidated_data,
                output_dir,
                f"{base_name}_geo_matrix_consolidated.json"
            )
        ...
        final_payload = {
            ...
            "outputs": {
                ...
                "geo_matrix_consolidated_path": geo_matrix_consolidated_path
            }
        }
