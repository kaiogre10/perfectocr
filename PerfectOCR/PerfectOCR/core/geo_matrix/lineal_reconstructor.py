...
class LineReconstructor:
    def __init__(self, page_width: int, page_height: int, config_line_reconstructor_params: Dict):
        ...
        # Nuevo parámetro: diferencia máxima de centroides (en fracción de la altura mínima) para agrupar
        self.centroid_y_gap_ratio = float(config_line_reconstructor_params.get('centroid_y_gap_ratio', 0.3))
    ...
    def _calculate_vertical_overlap_ratio(self, el1: Dict, el2: Dict) -> bool:
        ...
        # Segundo criterio: diferencia de centroides Y (para evitar agrupar líneas muy próximas pero distintas)
        c1_y = float(el1.get('cy', (y1_min + y1_max) / 2.0))
        c2_y = float(el2.get('cy', (y2_min + y2_max) / 2.0))
        centroid_gap = abs(c1_y - c2_y)
        centroid_gap_ratio = centroid_gap / max(min_h, 1e-5)

        return (overlap_rel >= self.vertical_overlap_threshold) and (centroid_gap_ratio <= self.centroid_y_gap_ratio)
